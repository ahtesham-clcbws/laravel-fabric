...
class Table extends Component
{
    ...
    /**
     * 🧬 FORGED BY FABRIC
     * Handles state for searching, sorting, and filtering Faq records.
    ...
    protected $listeners = ['faq-saved' => '$refresh'];
    ...
    public function mount()
    {
        ...
        if ($recordId = request('record')) {
             $this->dispatch('openModal', [
                 'component' => 'App\Livewire\Fabric\Faq.FaqEditor',
        ...
        return Faq::query()
            ...
            ->when($this->search, function ($query) {
                $query->where(function ($q) {
                    $q->orWhere('question', 'like', "%{$this->search}%");
                    $q->orWhere('answer', 'like', "%{$this->search}%");
    ...
    {
        return view('livewire.fabric.faq.table', [
    ...
    {
        $filename = "Faq_" . now()->format('Y-m-d_His') . ".csv";
        $headers = [
        ...
        $columns = ['id', 'question', 'answer', 'created_at']; // Customizable
