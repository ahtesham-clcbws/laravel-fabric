...
class Table extends Component
{
    ...
    /**
     * 🧬 FORGED BY FABRIC
     * Handles state for searching, sorting, and filtering Testimonial records.
    ...
    protected $listeners = ['testimonial-saved' => '$refresh'];
    ...
    public function mount()
    {
        ...
        if ($recordId = request('record')) {
             $this->dispatch('openModal', [
                 'component' => 'App\Livewire\Fabric\Testimonial.TestimonialEditor',
        ...
        return Testimonial::query()
            ...
            ->when($this->search, function ($query) {
                $query->where(function ($q) {
                    $q->orWhere('author', 'like', "%{$this->search}%");
                    $q->orWhere('content', 'like', "%{$this->search}%");
                    $q->orWhere('avatar', 'like', "%{$this->search}%");
    ...
    {
        return view('livewire.fabric.testimonial.table', [
    ...
    {
        $filename = "Testimonial_" . now()->format('Y-m-d_His') . ".csv";
        $headers = [
        ...
        $columns = ['id', 'author', 'content', 'avatar', 'created_at']; // Customizable
