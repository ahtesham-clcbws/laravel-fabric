    ...
    <div class="card-body">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
            <div class="form-control w-full max-w-xs">
                <div class="join">
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search inquiries..." class="input input-bordered join-item w-full" />
        ...
        <div class="overflow-x-auto rounded-box">
            <table class="table table-zebra w-full">
                ...
                <th class="">Email</th>
                @endif
                @if($columnVisibility['subject'] ?? true)
                <th class="">Subject</th>
                @endif
                @if($columnVisibility['message'] ?? true)
                <th class="">Message</th>
                @endif
                ...
                <tbody wire:loading.class="opacity-50">
                    @forelse($rows as $row)
                        ...
                        @endif
                            ...
                            <td class="text-right">
                                 <button wire:click="$dispatch('openModal', { component: 'fabric.inquiry.editor', arguments: { record: {{ $row->id }} } })" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="100%" class="text-center py-10 text-base-content/50 italic">No inquiries found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
