    ...
    <div class="card-body">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
            <div class="form-control w-full max-w-xs">
                <div class="join">
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search services..." class="input input-bordered join-item w-full" />
        ...
        <div class="overflow-x-auto rounded-box">
            <table class="table table-zebra w-full">
                ...
                <th class="">Title</th>
                @endif
                @if($columnVisibility['description'] ?? true)
                <th class="">Description</th>
                @endif
                @if($columnVisibility['icon'] ?? true)
                <th class="">Icon</th>
                @endif
                @if($columnVisibility['slug'] ?? true)
                <th class="">Slug</th>
                @endif
                ...
                <tbody wire:loading.class="opacity-50">
                    @forelse($rows as $row)
                        ...
                        @endif
                            ...
                            <td class="text-right">
                                 <button wire:click="$dispatch('openModal', { component: 'fabric.service.editor', arguments: { record: {{ $row->id }} } })" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="100%" class="text-center py-10 text-base-content/50 italic">No services found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
