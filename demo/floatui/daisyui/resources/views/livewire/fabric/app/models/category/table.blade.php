<div class="p-6 bg-white border-b border-gray-200 shadow-sm rounded-lg" x-data="{ showColumnPicker: false }">
    ...
    <div class="flex items-center justify-between mb-6 gap-4">
        <div class="flex-1 max-w-sm relative">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('Search') }} categories..." class="w-full border-gray-300 rounded-md shadow-sm pl-10 focus:border-indigo-600 focus:ring-indigo-600">
                ...
                @forelse($rows as $row)
                    <tr wire:key="row-{{ $row->id }}" class="{{ in_array($row->id, $selected) ? 'bg-indigo-50' : '' }} hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                             <button wire:click="$dispatch('openModal', { component: 'fabric.app.models.category.category-editor', arguments: { record: {{ $row->id }} } })" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="100%" class="px-6 py-10 text-center text-gray-500 italic">No categories found matching your search.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
