<div class="p-6 bg-white border-b border-gray-200 shadow-sm rounded-lg" x-data="{ showColumnPicker: false }">
    ...
    <div class="flex items-center justify-between mb-6 gap-4">
        <div class="flex-1 max-w-sm relative">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('Search') }} posts..." class="w-full border-gray-300 rounded-md shadow-sm pl-10 focus:border-indigo-600 focus:ring-indigo-600">
    ...
    <div class="relative overflow-x-auto">
        ...
        <table class="min-w-full divide-y divide-gray-200" wire:loading.class="opacity-50">
            <thead class="bg-gray-50">
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Is Published</th>
                <th class="px-6 py-3 text-left">
                    <button wire:click="sortBy('published_at')" class="group inline-flex items-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Published At
                        <span class="ml-2 flex-none rounded text-gray-400 group-hover:bg-gray-200">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 15L12 18.75 15.75 15m-7.5-6L12 5.25 15.75 9" /></svg>
                        </span>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Meta Data</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200" wire:loading.class="opacity-50">
                @forelse($rows as $row)
                    <tr wire:key="row-{{ $row->id }}" class="{{ in_array($row->id, $selected) ? 'bg-indigo-50' : '' }} hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $row->content }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $row->is_published }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">
                            {{ $row->published_at?->format('Y-m-d H:i') ?? '-' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{{ is_array($row->meta_data) ? json_encode($row->meta_data) : $row->meta_data }}</td>

                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            
            @if($row->trashed())
                <button wire:click="restore({{ $row->id }})" class="text-green-600 hover:text-green-900">Restore</button>
            @endif <button wire:click="$dispatch('openModal', { component: 'fabric.app.models.post.post-editor', arguments: { record: {{ $row->id }} } })" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="100%" class="px-6 py-10 text-center text-gray-500 italic">No posts found matching your search.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
