...
{
    public const VERSION = '1.4.5-beta';
    ...
    /**
     * The Sentinel: Automatic Crash Reporting
     */
    protected function registerSentinel(): void
    {
        if (!$this->app->runningInConsole() || !config('fabric.auto_report', true)) {
            return;
        }

        $this->app->make('events')->listen(ConsoleErrorEvent::class, function (ConsoleErrorEvent $event) {
            $command = $event->getCommand();
            
            // Only capture Fabric commands
            if (!$command || !str_starts_with($command->getName(), 'fabric:')) {
                return;
            }

            // Don't report the report command itself to avoid loops
            if ($command->getName() === 'fabric:report') {
                return;
            }

            $exception = $event->getError();
            $this->reportCrashToRegistry($command->getName(), $exception);
        });
    }

    protected function reportCrashToRegistry(string $commandName, \Throwable $e): void
    {
        try {
            $uuidPath = base_path('.fabric');
            $projectId = File::exists($uuidPath) ? \trim(File::get($uuidPath)) : null;

            $url = config('fabric.registry.url') . '/functions/v1/report';
    ...
    protected function registerCommands(): void
    {
        $this->commands([
            ...
            \CLCBWS\Fabric\Commands\ReportCommand::class,
        ]);
    }
